                    ...
                    if ($scope.type == 1) {
                        if (response != '' && response != null && response != undefined) {
                            ...
                            if ($scope.pay_type == 'weixin') { //如果是微信支付
                                ...
                                WXPaySer.setTotalCost($scope.earnest_cost-used_blc);
     ...
     function loadPageData(callback){
         ...
         OrderDetailsSer.requestData(order_id,function(resp,status){
             if(status==1){
                 ...
                 $scope.earnest_cost=Math.ceil($scope.data_order.UnitPrice*$scope.data_order.Count*$scope.data_order.EarnestPercent.toFixed(2))-$scope.data_order.EarnestMoney;//需支付的定金总额
                 callback();
             }
         });
